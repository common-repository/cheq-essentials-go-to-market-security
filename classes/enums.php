...
abstract class CHEGTMS_ErrorCodes
{
  const ERROR = 2000;
  const NO_KEYS = 2007;
...
abstract class CHEGTMS_AllowedCodes
{
  const VALID = 0;
...
abstract class CHEGTMS_BlockedCodes
{
  const SCRAPERS = 2;
  const NETWORK_ANOMALIES = 18;
  const FALSE_REPRESENTATION = 11;
  const AUTOMATION_TOOL = 3;
  const MALICIOUS_BOTS = 10;
  const EXCESSIVE_RATE_LIMIT = 6;
  const CLICK_HIJACKING = 17;
  const DISABLED_COOKIES = 16;
  const DISABLED_JS = 7;
  const CLICK_FARM = 9;
  const MALICIOUS_USER_INPUT = 25;
}

abstract class CHEGTMS_Urls
{
  const RTI_SERVER_EUROPE = 'https://rti-global.cheqzone.com/v1/realtime-interception';
  const RTI_LOGGER = 'https://rtilogger.production.cheq-platform.com/';
  const BOTZAPPING = 'https://botzapping.eu.cheq-platform.com';
  const CHEQ_TAG = 'obs.esnchocco.com';
  ...
  const CHEQ_ESSENTIAL_BOTZAPPING = 'https://cc-core.cheq-platform.com/api/bot-zapping/plugin';
  const HTTPS = 'https://';
}

abstract class CHEGTMS_HTTPCode
{
  const SUCCESS = 200;
  const BAD_REQUEST = 400;
}

abstract class CHEGTMS_ResponseMessage
{
  const INVALID_KEYS = 'Error: Please make sure you entered the right keys. Those can be found on your CHEQ Essential dashboard under the domain management page';
  const INVALID_DOMAIN = 'Error: Domain key is invalid';
  const SUCCESS = 'success';
}

abstract class CHEGTMS_DomainState
{
