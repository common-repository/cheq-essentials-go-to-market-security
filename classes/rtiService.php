...
const BLOCKED_CODES = [
    CHEGTMS_BlockedCodes::AUTOMATION_TOOL,
    CHEGTMS_BlockedCodes::CLICK_FARM,
    CHEGTMS_BlockedCodes::CLICK_HIJACKING,
    CHEGTMS_BlockedCodes::DISABLED_COOKIES,
    CHEGTMS_BlockedCodes::DISABLED_JS,
    CHEGTMS_BlockedCodes::EXCESSIVE_RATE_LIMIT,
    CHEGTMS_BlockedCodes::FALSE_REPRESENTATION,
    CHEGTMS_BlockedCodes::MALICIOUS_BOTS,
    CHEGTMS_BlockedCodes::MALICIOUS_USER_INPUT,
    CHEGTMS_BlockedCodes::NETWORK_ANOMALIES,
    CHEGTMS_BlockedCodes::SCRAPERS
];
...
class CHEGTMS_RTI
{
    public function is_monitoring_with_botzapping($api_key, $tag_hash, $secret)
    {
        ...
        $url = CHEGTMS_Urls::BOTZAPPING . '/plugin/monitoring';
    ...
    public function auth_with_botzapping($api_key, $tag_hash, $secret)
    {
        ...
        $apiResponse = wp_remote_post(
            CHEGTMS_Urls::BOTZAPPING . '/authorize/plugin',
            $request
        );
        ...
        if ($response_code !== CHEGTMS_HTTPCode::SUCCESS && !is_wp_error($apiResponse)) {
            $this->send_error($request, $response_body);
        }

        return $response_code !== CHEGTMS_HTTPCode::SUCCESS ? "" : $response_body;
    }
    ...
    public function auth_with_rti($api_key, $request_url, $event_type, $tag_hash)
    {
        ...
        try {
            $request = $this->create_rti_request(
                $api_key,
                $request_url,
                $event_type,
                $tag_hash
            ...
            $apiResponse = wp_remote_post(CHEGTMS_Urls::RTI_SERVER_EUROPE, $request);
            ...
            if (!$error_message) {
                ...
                if (
                    $response_code !== CHEGTMS_HTTPCode::SUCCESS &&
                    !is_wp_error($apiResponse)
                ) {
                    $this->send_error($request, $response_body);
                } elseif ($response_code == CHEGTMS_HTTPCode::SUCCESS) {
        ...
        if (!empty($response)) {
            ...
            if (!isset($response->setCookie)) {
                ...
                if (
                    ...
                    CHEGTMS_LogService::log(
                        ...
                        "",
                        "",
                        CHEGTMS_ErrorCodes::RTI_SERVER_INCORRECT_COOKIE
                    );
                }
            }
            ...
            if (!isset($response->version) || !is_numeric($response->version)) {
                ...
                CHEGTMS_LogService::log(
                    ...
                    "",
                    "",
                    CHEGTMS_ErrorCodes::RTI_SERVER_AUTH_ERROR
                );
            } else {
            ...
            if (!isset($response->threatTypeCode)) {
                CHEGTMS_LogService::log(
                    ...
                    "",
                    "",
                    CHEGTMS_ErrorCodes::RTI_SERVER_MISSING_THREAT_TYPE
                );
            } elseif (
                in_array($response->threatTypeCode, BLOCKED_CODES) &&
                $response->isInvalid
            ) {
                $res = [
                    "is_valid" => false,
                ...
                if (!isset($response)) {
                    CHEGTMS_LogService::log(
                        ...
                        "",
                        "",
                        CHEGTMS_ErrorCodes::RTI_SERVER_EMPTY_RESPONSE_FORMAT
                    );
                    ...
                    CHEGTMS_LogService::log(
                        ...
                        "",
                        "",
                        CHEGTMS_ErrorCodes::RTI_SERVER_INVALID_RESPONSE_FORMAT
                    );
                }
            }
        } else {
    ...
    {
        $action = sanitize_text_field(isset($_GET['clickcease']) ? $_GET['clickcease'] : '');
        if ($action == 'valid') {
            return;
        }
        ...
        $cheq_hash = sanitize_text_field(isset($_POST['cheq_hash']) ? $_POST['cheq_hash'] : '');
        if (empty($cheq_hash)) {
            echo json_encode([
                "status" => 400,
                "error" => "No hash",
        ...
        if (count($output) < 4) {
            ...
            CHEGTMS_LogService::log(
                ...
                "",
                "",
                CHEGTMS_ErrorCodes::INCORRECT_SECRET_KEY,
                $msg
            );
        } else {
            ...
            if (!$is_monitoring) {
                ...
                } elseif (in_array($output[2], BLOCKED_CODES) && $output[1]) {
    ...
    public function updateUserStatus($api_key, $client_id, $status)
    {
        ...
        // add route
        $apiResponse = wp_remote_post(
            CHEGTMS_Urls::CLICKCEASE_BOTZAPPING . '/State',
            $request
        );

        $response_code = $this->get_response_code($request, $apiResponse);

        return $response_code == CHEGTMS_HTTPCode::SUCCESS;
    }

    public function update_cheq_essential_status(
        $status
    ) {
        $data = new stdClass();
        $data->state = $status;
        ...
        $api_key = get_option('clickcease_api_key', '');
        $secret_key = get_option('clickcease_secret_key', '');
        $domain_key = get_option('clickcease_domain_key', '');
        $request = $this->get_cheq_essential_request(
            $api_key,
            $secret_key,
            $domain_key,
            $dataStr
        ...
        // add route
        $apiResponse = wp_remote_post(
            CHEGTMS_Urls::CHEQ_ESSENTIAL_BOTZAPPING . '/state',
            $request
        );
        $response_code = $this->get_response_code($request, $apiResponse);
    ...
    public function update_bz_domain(
        $api_key,
        $client_id,
        $plugin_latests_version
    ) {
        $data = new stdClass();
        $data->Address = Utils::getDomain();
        ...
        // add route
        $apiResponse = wp_remote_post(
            CHEGTMS_Urls::CLICKCEASE_BOTZAPPING . '/update',
            $request
        );
        $response_code = $this->get_response_code($request, $apiResponse);
    ...
    public function update_cheq_essential_domain(
        $plugin_latests_version
    ) {
        $api_key = get_option('clickcease_api_key', '');
        $secret_key = get_option('clickcease_secret_key', '');
        $domain_key = get_option('clickcease_domain_key', '');
        $data = new stdClass();
        $data->latestVersion = $plugin_latests_version;
        $dataStr = json_encode($data);
        $request = $this->get_cheq_essential_request(
            $api_key,
            $secret_key,
        ...
        // add route
        $apiResponse = wp_remote_post(
            CHEGTMS_Urls::CHEQ_ESSENTIAL_BOTZAPPING . '/status',
            $request
        );
    ...
    public function get_active_domain($api_key)
    {
        ...
        $apiResponse = wp_remote_get(CHEGTMS_Urls::BOTZAPPING . '/plugin/active-domain?appID=2', $request);
        $error_message = "";
        ...
        if (!$error_message && $status_code == CHEGTMS_HTTPCode::SUCCESS) {
    ...
    private function get_cheq_essential_request(
        $api_key,
        $secret_key,
        $domain_key,
        $dataStr
    ...
    private function create_rti_request(
        $api_key,
        $request_url,
        $event_type,
        $tag_hash
        ...
        if ($event_type == 'page_load') {
            ...
            $request_params['Connection'] = Utils::getServerVariable(
                'HTTP_CONNECTION'
            );
        }
...
require_once CHEGTMS_PLUGIN_PATH . 'classes/utils.php';
require_once CHEGTMS_PLUGIN_PATH . 'classes/enums.php';
